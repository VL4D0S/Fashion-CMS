    ...
    shopList.addEventListener("click", (evt) => {
        ...
        if (
            prod.some(
                (pathItem) =>
                    pathItem.classList &&
                    pathItem.classList.contains("shop__item")
            )
        ) {
            ...
            buttonOrder.addEventListener("click", (evt) => {
                ...
                const inputs = Array.from(
                    shopOrder.querySelectorAll("[required]")
                );

                inputs.forEach((inp) => {
                    if (!!inp.value) {
                        if (
                            inp.classList.contains("custom-form__input--error")
                        ) {
...
if (pageOrderList) {
    pageOrderList.addEventListener("click", (evt) => {
        if (
            ...
            evt.target.classList.contains("order-item__toggle")
        ) {
            var path = evt.path || (evt.composedPath && evt.composedPath());
            Array.from(path).forEach((element) => {
                if (
                    element.classList &&
                    element.classList.contains("page-order__item")
                ) {
        ...
        if (
            evt.target.classList &&
            evt.target.classList.contains("order-item__btn")
        ) {
            const status = evt.target.previousElementSibling;

            if (
                status.classList &&
                status.classList.contains("order-item__info--no")
            ) {
...
if (productsList) {
    productsList.addEventListener("click", (evt) => {
        ...
        if (
            target.classList &&
            target.classList.contains("product-item__delete")
        ) {
...
if (document.querySelector(".shop-page")) {
    $(".range__line").slider({
        ...
        stop: function (event, ui) {
            $(".min-price").text(
                $(".range__line").slider("values", 0) + " руб."
            );
            $(".max-price").text(
                $(".range__line").slider("values", 1) + " руб."
            );
        },
